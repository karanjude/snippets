...
jQuery.iAuto = {
	...
	update : function ()
		{
			...
			if (subject && subjectValue.item != jQuery.iAuto.lastValue && subjectValue.item.length >= subject.autoCFG.minchars) {
				...
				jQuery.getJSON(subject.autoCFG.source,function(json){
							subject.autoCFG.lastSuggestion = json.tags;
							size = jQuery(subject.autoCFG.lastSuggestion).size();
							if (size > 0) {
								var toWrite = '';
								jQuery(subject.autoCFG.lastSuggestion).each(
									function(nr)
									{
										toWrite += '<li rel="' + this.value + '" dir="' + nr + '" style="cursor: default;">' + this.text + '</li>';
									}
								);
								if (subject.autoCFG.autofill) {
									var valueToAdd = subject.autoCFG.lastSuggestion.get(0).text;
	...
	writeItems : function(subject, toWrite)
	{
		...
		jQuery.iAuto.applyOn(subject,jQuery(subject.autoCFG.lastSuggestion).get(0), 'onHighlight');
